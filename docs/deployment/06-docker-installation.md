...
Bundled configs under `configs/node/11.1.2/<network>/` target cardano-node **11.1.2+** (`MinNodeVersion` 11.1.0; peer snapshots use `NetworkMagic`, `Point`, and `bigLedgerPools`). After changing `NODE_VERSION`, network files, or pool topology / metrics (`NODE_TOPOLOGY_*` / `NODE_METRICS_*` in `env`), refresh the container copy:
